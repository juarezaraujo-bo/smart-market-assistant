...
const AlertItem = ({ type, message, time, status }: AlertItemProps) => {
  ...
  const isPanelOnly = status === 'skipped' || status === 'panel_only';
  const badgeClass = status === 'sent' || isPanelOnly ? 'badge-success' : 'badge-warning';
  const badgeLabel = isPanelOnly
    ? 'Painel'
    : status === 'pending'
      ? 'WhatsApp: Pendente'
      : status === 'processing'
        ? 'WhatsApp: Processando'
        : status === 'sent'
          ? 'WhatsApp: Enviado'
          : 'WhatsApp: Falhou';
    ...
    <div className="card" style={{ marginBottom: '12px', borderLeft: `4px solid ${borderColors[type || ''] || 'var(--border)'}` }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        ...
        <div style={{ textAlign: 'right' }}>
          <div className={`badge ${badgeClass}`} style={{ fontSize: '10px' }}>
            {badgeLabel}
          </div>
        </div>
      </div>
    ...
    <div className="animate-fade" style={{ maxWidth: '800px' }}>
      <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '32px' }}>
        <div>
          ...
          <p style={{ color: 'var(--text-muted)', fontSize: '14px' }}>Alertas de inteligência gerados automaticamente após cada importação.</p>
        </div>
      </header>
      ...
      {loading ? (
        <div style={{ textAlign: 'center', padding: '60px' }}>
          ...
          <p style={{ marginTop: '16px', color: 'var(--text-muted)', fontSize: '14px' }}>Buscando alertas de inteligência...</p>
        </div>
      ) : error ? (
        <div className="card" style={{ border: '1px solid var(--danger)', background: 'rgba(239, 68, 68, 0.05)', textAlign: 'center', padding: '40px' }}>
          ...
          <h3 style={{ fontSize: '16px', color: 'var(--danger)', marginBottom: '8px' }}>Erro de Conexão</h3>
        ...
        <div className="card" style={{ textAlign: 'center', padding: '60px 20px', color: 'var(--text-muted)' }}>
          ...
          <p style={{ fontSize: '14px' }}>Sua central de inteligência está limpa. Novos alertas aparecerão após a próxima importação de dados.</p>
        </div>
      )}
    </div>
  );
